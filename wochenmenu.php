...
// Menüverzeichnis
$menudir = __DIR__ . '/listen/menuhistorie/';
...
function ladeProduktListe() { return Promise.resolve(window.produktListe); }
      ...
      <link href="css/wochenmenu.css" rel="stylesheet">
    </head>
    <body>
      ...
      <!-- Modal -->
      <?php include 'rezeptkasten/rezept-modal.html'; ?>

      <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
      <script src="rezeptkasten/rezept-modal.js"></script>
      <script>
        ...
        function showRezeptModal(slug) {
            const r = rezepte[slug];
            if (!r) return;
            const titel = r.titel;
            const bild = r.bild;
            const vegetarisch = r.vegetarisch === 'ja';
            const json = r.json || {};
            document.getElementById('modalRezeptTitel').innerHTML = titel + (vegetarisch ? ' <span class="modal-vegan-icon material-symbols-outlined" title="Vegetarisch">eco</span>' : '');
            let html = '';
            if (bild) html += `<img src="${bild}" class="modal-img mb-3" alt="">`;
            if (json.zubereitung && json.zubereitung.length) {
            ...
            if (json.zutaten && json.zutaten.length) {
                ...
                if (Array.isArray(json.zutaten)) {
                    ...
                    html += `<li class="zutaten-normal">${resolveZutat(json.zutaten)}</li>`;
                }
                html += `</ul>`;
            }
            if (json.kalorien) {
...
<body>
  ...
  <script src="/einkauf-app/produkte/rezept-modal.js"></script>
</body>
</html>
