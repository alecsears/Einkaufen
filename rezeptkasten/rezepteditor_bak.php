...
// Rezept-Initialisierung (bleibt unverändert)
$rezeptname = '';
$zutaten    = [];
$bildname   = '';
$zubereitung = '';
    ...
    if (file_exists($pfad)) {
        ...
        if ($json) {
            ...
            foreach (($json['zutaten'] ?? []) as $z) {
                ...
                if (isset($z['rezepteinheit']) && $z['rezepteinheit'] !== '' && isset($produkteinheiten[$z['rezepteinheit']])) {
        ...
        $bildname = __DIR__ . "/bilder/" . pathinfo($dateiname, PATHINFO_FILENAME) . ".jpg";
    }
}
...
<body>
  <div class="container py-4">
    ...
    <form id="rezeptForm" action="speichere_rezept.php" method="post" enctype="multipart/form-data">
      ...
      <!-- ZUBEREITUNG: Neue Bootstrap-Form mit Formatierungshilfen -->
      <div class="mb-4">
      ...
      <!-- Bild-Upload: Nur Button, kein Dateiname -->
      <div class="mb-4">
        <label for="bild" class="form-label">Bild (JPG, optional)</label>
        <div class="d-flex align-items-center">
          <input type="file" name="bild" id="bild" class="d-none" accept="image/jpeg" onchange="previewBild()">
        ...
        <div id="bildPreview" class="mt-2">
          <?php if ($bildname && file_exists($bildname)): ?>
            <img src="<?= htmlspecialchars('bilder/' . pathinfo($dateiname, PATHINFO_FILENAME) . '.jpg') ?>?t=<?= time() ?>" class="img-thumbnail mt-3" style="max-width:200px;" alt="Rezeptbild">
          <?php endif; ?>
        </div>
      </div>
  ...
  <div class="sticky-footer d-flex justify-content-between align-items-center">
    ...
    <?php if (isset($_GET['edit'])): ?>
      <form action="loesche_rezept.php" method="post" class="d-inline" onsubmit="return confirm('Rezept wirklich löschen?');" style="margin:0;">
  ...
  <script>
    ...
    function formatZubereitung(action) {
      ...
      if (action === 'bold') {
        ...
        // Zeilen markieren, jede Zeile mit - beginnen
        ...
        // Zeilen markieren, jede Zeile mit 1. beginnen
        let lines = selected || 'Element';
        if (!selected) {
  ...
  // Produkt suchen (egal ob Name oder ID)
  if (zutatName) {
  ...
  // Nun selektieren, falls vorhanden
  if (
    preferredProdUnit &&
